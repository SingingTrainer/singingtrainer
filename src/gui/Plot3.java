 ...
 *
 */
public class Plot3 extends JPanel
{
	/** the frame **/
	 ...
	 */
	public Plot3( final String title, final int width, final int height )
	{
...
//					panel.setPreferredSize( new Dimension( width, height ) );
	...
	{			
		...
		{						
			...
			{
				image = new BufferedImage( /*(int)(samples.length / samplesPerPixel)*/700, this.getHeight(), BufferedImage.TYPE_4BYTE_ABGR );
				Graphics2D g = image.createGraphics();
				g.setColor( Color.black );
				g.fillRect( 0, 0, image.getWidth(), image.getHeight() ); 
				g.dispose();
	...
	public void plotInt( List<Integer> samples, final float samplesPerPixel, final Color color )
	{			
		synchronized( image )
		{						
			if( image.getWidth() <  samples.size() / samplesPerPixel )
	...
	public void plotInt2( float[] samples, final float samplesPerPixel, final float offset, final boolean useLastScale, final Color color )
	{
		synchronized( image )
		{
			if( image.getWidth() <  samples.length / samplesPerPixel )
	...
	public void plotInt3( List<Integer> samples, final float samplesPerPixel, final float offset, final boolean useLastScale, final Color color )
	{
		synchronized( image )
		{
			if( image.getWidth() <  samples.size() / samplesPerPixel )
